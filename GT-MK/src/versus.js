...
import { getCurrentUser, listProfiles, getProfile, getParentName, getParentAvatar, isParent } from "./accounts.js";
...
// هويّةُ **صاحبِ حسابِ الوالدَين** حين يلعبُ بنفسِه (أب/أم): تُسألُ مرّةً وتُحفَظ، فيَنفصلُ سجلُّ
// الأبِ عن سجلِّ الأمّ. وحين يلعبُ الوالدُ «ضدّ الوالد» يكونُ الخصمُ **الطرفَ الآخرَ تلقائيًّا**
// (لا معنى لسؤالِ الطرفَينِ معًا)؛ وهذا يُصلحُ أيضًا تطابقَ الاسمَينِ الذي كان يُفسِدُ تمييزَ الفائز.
const PSELF_KEY = "tilmithi_vs_parentself";
function loadSelfFemale() { try { return localStorage.getItem(PSELF_KEY) === "f"; } catch (e) { return false; } }
export const PARENT_SELF = { female: loadSelfFemale(), chosen: (() => { try { return !!localStorage.getItem(PSELF_KEY); } catch (e) { return false; } })() };
export function setParentSelf(f) {
  PARENT_SELF.female = !!f; PARENT_SELF.chosen = true;
  try { localStorage.setItem(PSELF_KEY, f ? "f" : "m"); } catch (e) {}
}
export const parentLabel = f => (f ? "الأم" : "الأب");
export const parentAvatarOf = f => (f ? "👩" : "👨");
...
function meCard() {
  ...
  // حسابُ الوالدَين هويّةٌ واحدةٌ اسمُها «الوالدان» — نُسمّيه أبًا أو أمًّا ليَنفصلَ سجلُّ كلٍّ منهما.
  if (u && u.role === "parent") return { name: parentLabel(PARENT_SELF.female), avatar: parentAvatarOf(PARENT_SELF.female), female: PARENT_SELF.female };
  return (u && u.name) ? { name: u.name, avatar: u.avatar || "🙂", female: u.gender === "f" } : { name: "أنت", avatar: "🧒", female: false }; }
...
export function vsMakePlayers(forceTwo) {
  ...
  if (VS.opp === "parent") {
    // إن كان اللاعبُ نفسُه هو الوالد ⇒ الخصمُ هو **الطرفُ الآخر** حتمًا (أبٌ ضدّ أمّ) بلا سؤال.
    if (isParent()) { const o = !PARENT_SELF.female; return [m, { name: parentLabel(o), avatar: parentAvatarOf(o), role: "parent", score: 0, cpu: false, female: o, color: VS_COLB }]; }
    return [m, { name: getParentName(), avatar: getParentAvatar(), role: "parent", score: 0, cpu: false, female: VS.parentFemale, color: VS_COLB }];
  }
...
export function vsSetup(host, cfg) {
  ...
  host.innerHTML = `<div class="clue" style="padding:12px"><div class="say">${cfg.say}</div></div>
    <div class="vs-set">
      ...
      ${isParent() ? `<div class="vs-row"><div class="vs-lbl">👤 مَن أنت؟ (ليَنفصلَ سجلُّ كلٍّ منكما)</div><div class="vs-lvl" id="vsSELF"><button data-self="m" class="${!PARENT_SELF.female ? "on" : ""}">👨 الأب</button><button data-self="f" class="${PARENT_SELF.female ? "on" : ""}">👩 الأم</button></div></div>` : ""}
      <div class="vs-row" id="vsPR" style="${VS.opp === "parent" && !isParent() ? "" : "display:none"}"><div class="vs-lbl">👤 مَن سيلعب؟ (ليخاطبَه الآليُّ صحيحًا)</div><div class="vs-lvl" id="vsP"><button data-pg="m" class="${!VS.parentFemale ? "on" : ""}">👨 أب</button><button data-pg="f" class="${VS.parentFemale ? "on" : ""}">👩 أم</button></div></div>
  ...
  // هويّةُ صاحبِ حسابِ الوالدَين (أب/أم): تُحفَظُ وتُغيّرُ اسمَه في السجلِّ وخطابَ الآليِّ له.
  const vself = host.querySelector("#vsSELF"); if (vself) vself.addEventListener("click", e => { const b = e.target.closest("[data-self]"); if (!b) return; setParentSelf(b.getAttribute("data-self") === "f"); rr(); });
