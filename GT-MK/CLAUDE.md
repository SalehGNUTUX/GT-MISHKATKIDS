...
قسمٌ مستقلّ: بطاقة الفهرس «🌍 لغات أجنبيّة» → `lang.html` (اختيار en/fr ثمّ أقسامٌ بمعاملات URL `?lng=&sec=&cat=&story=&pg=`). أقسامٌ: **الأحرف · الأرقام · الكلمات (9 فئات مواضيعيّة، كلمة+معنى عربيّ+رمز) · القصص المصوّرة**. `content/lang-<code>.js` يصدّر `{code, name, nameAr, flag, dir, voice, letters, numbers, words, stories}`. **القصص = ترجمةُ `content/stories.js`** بنفس `id`/`cover`/`art` (تُعاد رسومُها من `src/story-art.js`) — فأيُّ قصّةٍ عربيّةٍ جديدةٍ تُضاف ترجمتُها هنا بنفس المفاتيح. قارئُ القصّة الكاملة بزرّ «📖 القصّة كاملةً» (تتابعٌ مع تقليب + إيقاف). **النطق:** عصبيٌّ Piper مُدمَجٌ offline — `en_GB-alba`→مجموعة `tts-alba`، `fr_FR-tom`→`tts-tom` (تُولَّد بـ`tools/gen-piper-lang.mjs en|fr`، النماذج `.onnx*` مُستثناةٌ من git، المقاطعُ تُرفَع). `lang.html` ينطق عبر `playClipForSet(text, L.voice)` و`playClipForSetAsync` (للقراءة المتتابعة)، وإلّا Web Speech بلغة القسم. المجموعتان `types:["foreign"]` فلا تظهران في قوائم الصوت العربيّة. نصوصُ اللغة LTR ضمن واجهةٍ RTL (صنف `.ltr`). `piper_synth.py` يقبل معامِلَ متحدّثٍ رابعاً (للنماذج متعدّدة الأصوات). **المتبقّي:** نصوص + اختبارات + ألعاب اللغة.
