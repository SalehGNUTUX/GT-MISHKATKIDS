...
| 🎮 ألعاب وأنشطة | `play.html` | **لوحةُ أقسام**: بطاقةُ صنفٍ تفتحُ ألعابَه وحدَها (الترفيهيّةُ أوّلًا). **10 ألعابٍ تنافسيّةٍ** ضدّ الآليّ (بثلاث صعوبات) أو الإخوةِ أو الوالد — منها **أربعةٌ على التوالي** و**مربّعاتُ النقاط** و**دائرةٌ وعلامة** و**الذاكرة** و**تحدّي الأعداد**، بردودٍ منطوقةٍ و**«⏱️ ضدّ الوقت»** و**الفائزُ يبدأُ الجولةَ التالية** و**سجلِّ مبارياتٍ** في لوحةِ الأهل. + ألعابُ دمجِ حروفٍ + ألعاب/أنشطة للطباعة (أصيلة). |
...
## 📚 المحتوى
- **مجموعة GT-SARARIM (محوَّلة):** 51 قصّة (أخلاق · قرآن · أنبياء · علماء · صحابة) + 102 سؤال فهم · 66 لغزًا (فوازير ومنطق) · 99 سؤال اختبار — **نصوصها منقولةٌ من [GT-SARARIM](https://github.com/SalehGNUTUX/GT-SARARIM) (GPL-3.0)**، انظر [`docs/CREDITS.md`](docs/CREDITS.md).
- **مكتبة الذكريات:** 85 ذكرية في 5 مجالات (أرقام وهندسة · حروف ولغة · أخلاق ومشاعر · علوم وطبيعة · تفكير وحلّ مشكلات) × فئتين عمريّتين (4–6 و7–10) — منها: 9 مشاريع متعدّدة الأيّام · 5 موسميّة · 6 تعاونيّة للإخوة · 5 مهارات حياتية · 5 أمان وصحّة · 5 قدوات وقيم.
- **القصص المصوّرة:** 9 قصص بقيمٍ مختلفة — المثابرة · الصبر · الشجاعة · الصدق · التعاون · الفضول · الرحمة بالحيوان · النظافة.
...
**أدواتٌ وقتَ التطوير** (اختياريّةٌ، تُشغَّل عند تغيير المحتوى أو الأيقونات):
```bash
npm run check:icons   # يفشل إن عُرِض رمزٌ خامًّا بدل الأيقونة المعتمدة
npm run gen:icons     # يولّد src/icons.js من tools/gen-fa-icons.mjs (لا تُحرّره يدويًّا)
npm run gen:piper     # ينطق النصوصَ الجديدة بالصوت العصبيّ (تفاضليّ: يتخطّى الموجود)
npm run gen:audio     # مقاطع espeak للمفردات
```
> أيُّ نصٍّ منطوقٍ جديدٍ يحتاج `gen:piper` وإلّا نُطِق بصوت النظام. والحزمُ تُبنى من **شجرة العمل** لا من git — فتأكّدْ أنّ `git status` نظيفةٌ قبل التحزيم.
